...
void reliefkarte_t::calc_map()
{
	...
	if(mode==MAP_TOURIST) {
		...
		FOR(weighted_vector_tpl<gebaeude_t*>, const i, ausflugsziele) {
			int const pax = i->get_visitor_demand_per_workday();
		...
		FOR(weighted_vector_tpl<gebaeude_t*>, const g, ausflugsziele) {
			...
			set_relief_farbe_area( pos, 7, calc_severity_color(g->get_visitor_demand_per_workday(), max_tourist_ziele));
		}
		return;
	}
	...
	if(  mode & MAP_TOURIST  ) {
		FOR(  weighted_vector_tpl<gebaeude_t*>, const gb, welt->get_ausflugsziele()  ) {
			if(  gb->get_first_tile() == gb  ) {
				...
				gb_pos = gb_pos + pos;
				int const pax = gb->get_visitor_demand_per_workday();
				if(  max_tourist_ziele < pax  ) {
					max_tourist_ziele = pax;
				}
				COLOR_VAL color = calc_severity_color_log(gb->get_visitor_demand_per_workday(), max_tourist_ziele);
