...
void karte_t::rotate90_world_lists()
{
	for(int i=0; i<passenger_origins.get_count(); i++) {
		passenger_origins[i] = passenger_origins[i]->get_first_tile();
	}
	for(int i=0; i<commuter_targets.get_count(); i++) {
		commuter_targets[i] = commuter_targets[i]->get_first_tile();
	}
	for(int i=0; i<visitor_targets.get_count(); i++) {
		visitor_targets[i] = visitor_targets[i]->get_first_tile();
	}
	for(int i=0; i<mail_origins_and_targets.get_count(); i++) {
		mail_origins_and_targets[i] = mail_origins_and_targets[i]->get_first_tile();
	}
}
...
{
	assert(gb != NULL);
	ausflugsziele.append(gb, gb->get_visitor_demand_per_workday());
...
int karte_t::count_larger_cities(sint64 pop)
{
	int count = 0;
	FOR(weighted_vector_tpl<stadt_t*>, const s, stadt) {
		if(s->get_einwohner() > pop) {
			count++;
		}
	}
	return count;
...
void karte_t::generate_passengers_or_mail(const ware_besch_t * wtyp)
{
	...
	/* weight is already scaled by get_population_per_workday, so we don't need to increase passengers, too */
	const int num_pax = 1;
	...
	current_destination.location = origin_pos;
	destination first_destination = current_destination;
...
karte_t::destination karte_t::find_destination(trip_type trip)
{
	...
	current_destination.building = NULL;
	gebaeude_t* gb = NULL;
	...
	case commuting_trip: 
		if (!commuter_targets.empty()) {
			gb = pick_any_weighted(commuter_targets);
		}
		break;

	case visiting_trip:
		if (!visitor_targets.empty()) {
	...
	default:
	case mail_trip:
		if (!mail_origins_and_targets.empty()) {
...
int karte_t::count_commuters_to(koord pos)
{
	int count = 0;

	FOR(vector_tpl<convoihandle_t>, const cnv, convoi_array) {
		count += cnv->count_commuters_to(pos);
	}
...
const vector_tpl<const ware_besch_t*> &karte_t::get_goods_list()
{
	if (goods_in_game.empty()) {
		...
		goods_in_game.insert_at(1, warenbauer_t::commuters);
		goods_in_game.insert_at(2, warenbauer_t::post);
	}
...
#endif
	/* we get called twice for attractions inside a city, once from
	 * add_gebaeude_to_stadt and once from add_auflugsziel */
	/* It doesn't matter which interval we use, as long as we're consistent.
	 * The _per_workday versions have the best precision */
	passenger_origins.append(gb, gb->get_passenger_trips_per_workday());
	commuter_targets.append(gb, gb->get_jobs_per_workday());
	visitor_targets.append(gb, gb->get_visitor_demand_per_workday());
	mail_origins_and_targets.append(gb, gb->get_mail_demand_per_workday());
}
...
void karte_t::update_weight_of_building_in_world_list(gebaeude_t *gb)
{
	...
	if(passenger_origins.is_contained(gb))
	{
		passenger_origins.update(gb, gb->get_passenger_trips_per_workday());
	}

	if(commuter_targets.is_contained(gb))
	{
		commuter_targets.update(gb, gb->get_jobs_per_workday());
	}

	if(visitor_targets.is_contained(gb))
	{
		visitor_targets.update(gb, gb->get_visitor_demand_per_workday());
	}

	if(mail_origins_and_targets.is_contained(gb))
	{
		mail_origins_and_targets.update(gb, gb->get_mail_demand_per_workday());
	}
}

gebaeude_t *karte_t::random_residential_building()
{
	if(passenger_origins.get_sum_weight() == 0) {
		return NULL;
	} else {
		return pick_any_weighted(passenger_origins);
	}
}
